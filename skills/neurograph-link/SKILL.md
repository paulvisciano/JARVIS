...
description: Link between neurographs via memory-link nodes. Use when: (1) user wants to search across multiple neurographs, (2) comparing graph structures, (3) following memory-link connections. ALWAYS asks user before loading remote graphs (graphs are 100KB+ = ~50k tokens). Uses local_path when available (fast disk access). Falls back to neurograph-search for lightweight queries.
